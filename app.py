...
def enregistrer_sortie():
    try:
        ...
        code = data.get("codeProduit")
        date = data.get("dateHeure")

        if not code or not date:
            return jsonify({"erreur": "Champs codeProduit ou dateHeure manquants"}), 400

        nom_fichier = f"{code}_{date.replace(':', '').replace('-', '').replace('T', '_')[:15]}.json"
